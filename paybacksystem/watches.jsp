    ...
    <div class="art-layout-cell layout-item-2" style="width: 15%;">
      <p><font size="3" color="red">Shop Online</font></p>
		...
		<%
		  ...
		  while(rs.next())
		  {
		   comname1=rs.getString(1);
		  %>
		  <a href="<%=comname1%>.jsp">>&nbsp;&nbsp;&nbsp;<%=comname1%></a></br>
		 <% }
		%>
    </div>
	
	<%
	            String image="",proname="",pcost="",points="",comname="",warrenty="";
                ResultSet rs1=st.executeQuery("select * from products");
                while(rs1.next())
                 {	
	              comname=rs1.getString(1);
	              proname=rs1.getString(2);
	              pcost=rs1.getString(3);
				  warrenty=rs1.getString(4);
				  
	              points=rs1.getString(5);
	              image=rs1.getString(6);	  
	              session.setAttribute("HOME TOWN",comname);
	              %>
	<div class="art-layout-cell layout-item-2" style="width: 20%;">
				  
	   <a href="poductsbuy.jsp?images=<%="images/"+image%>&proname=<%=proname%>&pcost=<%=pcost%>&warrenty=<%=warrenty%>&points=<%=points%>">
	   <img width="180" height="122" alt="" src="images/<%=image%>"/></a></br>
		 summary:</br>
		   product  : <%=proname%></br>
		   cost     :<%=pcost%></br>
